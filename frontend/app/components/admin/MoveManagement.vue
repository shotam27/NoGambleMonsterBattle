        ...
        <div v-for="move in filteredMoves" :key="move._id" class="item-card">
          ...
          <div class="item-info">
            <div class="type-badges">
              <span class="type-badge" :class="`type-${move.type?.toLowerCase()}`">{{ move.type }}</span>
              <span class="category-badge" :class="`category-${move.category}`">{{ move.category }}</span>
            </div>
            ...
            <div v-if="move.description" class="description">{{ move.description }}</div>
            
            <div v-if="move.statusEffect" class="move-effects">
              <strong>状態異常:</strong> {{ getStatusEffectLabel(move.statusEffect) }}
            </div>
            
            <div v-if="move.statChange && move.statChange.stat" class="move-effects">
              <strong>能力変化:</strong> 
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveMove">
          ...
          <div class="form-section-inner">
            ...
            <div class="form-group">
              <label>
                <input type="checkbox" v-model="editingMove.switchAfterAttack" />
                とんぼがえり（攻撃後交代）
              </label>
            </div>
            <div class="form-group">
              <label>
                <input type="checkbox" v-model="editingMove.createsSubstitute" />
                身代わり
              </label>
            </div>
            <div class="form-group">
...
const getStatChangeLabel = (statChange) => {
  ...
  const stages = statChange.stages > 0 ? `+${statChange.stages}` : statChange.stages;
...
const saveMove = async () => {
  try {
    ...
    // 能力変化データを統合
    if (statChangeTarget.value && statChangeStat.value && statChangeStages.value) {
      moveData.statChange = {
    ...
    if (response.ok) {
      emit('message', isAddMode.value ? '技を追加しました' : '技を更新しました', 'success');
      await fetchMoves();
      cancelEdit();
    } else {
    ...
    emit('message', `技の${isAddMode.value ? '追加' : '更新'}に失敗しました`, 'error');
  }
};
...
const deleteMove = async (id) => {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/move/${id}`, { method: 'DELETE' });
    if (response.ok) {
