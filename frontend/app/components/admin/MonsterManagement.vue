<template>
  <div class="management-section">
    ...
    <div class="add-button-section">
      <button @click="openAddModal" class="btn-primary">新規モンスター追加</button>
    </div>
    ...
    <div class="list-section">
      ...
      <div class="search-box">
        <input v-model="searchQuery" type="text" placeholder="モンスター名で検索..." />
      </div>

      <div class="item-list">
        <div v-for="monster in filteredMonsters" :key="monster._id" class="item-card">
          ...
          <div class="item-info">
            <div class="type-badges">
              ...
              <span v-if="monster.type2" class="type-badge" :class="`type-${monster.type2?.toLowerCase()}`">
                {{ monster.type2 }}
              </span>
            </div>
            ...
            <div class="stats-grid">
              ...
              <div class="stat">特攻: {{ monster.stats?.magicAttack || 0 }}</div>
              <div class="stat">特防: {{ monster.stats?.magicDefense || 0 }}</div>
              <div class="stat">素早さ: {{ monster.stats?.speed || 0 }}</div>
            </div>

            <div v-if="monster.moves && monster.moves.length > 0" class="moves-info">
              <strong>技:</strong>
              <span v-for="(move, idx) in monster.moves" :key="move._id || idx">
                {{ typeof move === 'string' ? move : move.name }}<span v-if="idx < monster.moves.length - 1">, </span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveMonster">
          ...
          <div class="form-row stats-row">
            ...
            <div class="form-group">
              <label>特攻:</label>
              <input v-model.number="editingMonster.stats.magicAttack" type="number" required min="1" />
            </div>
            <div class="form-group">
              <label>特防:</label>
              <input v-model.number="editingMonster.stats.magicDefense" type="number" required min="1" />
            </div>
            <div class="form-group">
              <label>素早さ:</label>
          ...
          <div class="form-group">
            ...
            <div class="moves-selection">
              <div v-for="(move, index) in editingMonster.moves" :key="index" class="move-slot">
                <select v-model="editingMonster.moves[index]" class="move-select">
                  <option value="">-- 技を選択 --</option>
                  <option v-for="availMove in availableMoves" :key="availMove._id" :value="availMove._id">
                    {{ availMove.name }} ({{ availMove.type }})
                  </option>
                </select>
                <button type="button" @click="clearMoveSlot(index)" class="btn-clear" v-if="editingMonster.moves[index]">
                  ✕
                </button>
              </div>
            </div>
          ...
          <div class="modal-actions">
            <button type="submit" class="btn-primary">{{ isAddMode ? '追加' : '更新' }}</button>
...
const openAddModal = () => {
  ...
  editingMonster.value = {
    ...
    type1: '',
    type2: '',
    stats: { hp: 50, attack: 50, defense: 50, magicAttack: 50, magicDefense: 50, speed: 50 },
    moves: ['', '', '', '']
  };
};
...
const editMonster = (monster) => {
  ...
  // movesを4枠に調整
  const moveIds = (monsterCopy.moves || []).map(m => typeof m === 'string' ? m : m._id);
  monsterCopy.moves = [
    moveIds[0] || '',
    moveIds[1] || '',
    moveIds[2] || '',
...
const saveMonster = async () => {
  try {
    ...
    if (response.ok) {
      emit('message', isAddMode.value ? 'モンスターを追加しました' : 'モンスターを更新しました', 'success');
      await fetchMonsters();
      cancelEdit();
    } else {
      const error = await response.json();
      emit('message', `${isAddMode.value ? '追加' : '更新'}に失敗しました: ${error.message}`, 'error');
    }
  } catch (error) {
    console.error('Error saving monster:', error);
    emit('message', `モンスターの${isAddMode.value ? '追加' : '更新'}に失敗しました`, 'error');
  }
};
...
const deleteMonster = async (id) => {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/monster/${id}`, { method: 'DELETE' });
    if (response.ok) {
